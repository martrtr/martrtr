...
public final class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){ super.onCreate(b);
        ...
        audio=new AudioClient();audio.start(); kiosk=new RootKiosk();kiosk.start(this);
    ...
    private void gracefulExit(){if(exiting)return;exiting=true;if(kiosk!=null)kiosk.stop();if(net!=null)net.close();if(audio!=null)audio.close();new Handler(Looper.getMainLooper()).postDelayed(()->{finishAndRemoveTask();new Handler(Looper.getMainLooper()).postDelayed(()->android.os.Process.killProcess(android.os.Process.myPid()),200);},180);}
    @Override protected void onDestroy(){if(net!=null)net.close();if(audio!=null)audio.close();super.onDestroy();}
}
