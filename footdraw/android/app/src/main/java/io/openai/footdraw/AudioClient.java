...
public final class AudioClient implements AutoCloseable {
    ...
    private void net(){
        try{
            socket=new DatagramSocket();socket.setSoTimeout(100);socket.setReceiveBufferSize(32*1024);
            InetAddress host=InetAddress.getByName(HOST);long lastHello=0;byte[] buf=new byte[4096];
            while(running){
                long now=System.currentTimeMillis();
                if(now-lastHello>650){byte[] h=("FDH1"+TOKEN).getBytes(StandardCharsets.US_ASCII);socket.send(new DatagramPacket(h,h.length,host,PORT));lastHello=now;}
    ...
    private void parse(byte[] d){
        ...
        if(pcm.length>=28&&pcm[0]=='F'&&pcm[1]=='D'&&pcm[2]=='C'&&pcm[3]=='1'){
            try{
                int bg=((pcm[4]&255)<<16)|((pcm[5]&255)<<8)|(pcm[6]&255);int br=((pcm[7]&255)<<16)|((pcm[8]&255)<<8)|(pcm[9]&255);boolean land=pcm[10]!=0,clip=pcm[11]!=0;
                ByteBuffer c=ByteBuffer.wrap(pcm,12,16).order(ByteOrder.BIG_ENDIAN);float l=c.getFloat(),t=c.getFloat(),r=c.getFloat(),bt=c.getFloat();
        ...
        long now=System.currentTimeMillis();
        synchronized(lock){
            if((lastPacketMs>0&&now-lastPacketMs>250)||(expected>=0&&seq+4<expected)||(expected>=0&&seq>expected+24)){jitter.clear();expected=-1;}
            lastPacketMs=now;if(expected>=0&&seq<expected)return;jitter.put(seq,pcm);
            while(jitter.size()>6)jitter.pollFirstEntry();
            if(jitter.size()>=5){while(jitter.size()>2)jitter.pollFirstEntry();expected=jitter.firstKey();}
            lock.notifyAll();
        }
    }
    ...
    private void play(){
        ...
        AudioFormat af=new AudioFormat.Builder().setSampleRate(16000).setChannelMask(AudioFormat.CHANNEL_OUT_MONO).setEncoding(AudioFormat.ENCODING_PCM_16BIT).build();AudioTrack t=null;
        try{
            ...
            try{int cap=t.getBufferCapacityInFrames();if(cap>=320)t.setBufferSizeInFrames(320);}catch(Exception ignored){}
            if(android.os.Build.VERSION.SDK_INT>=31){try{t.setStartThresholdInFrames(Math.min(160,t.getBufferCapacityInFrames()));}catch(Exception ignored){}}
            t.setVolume(1.0f);t.play();byte[] silence=new byte[320];
            while(running){byte[] frame=null;synchronized(lock){if(expected<0&&jitter.size()>=2)expected=jitter.firstKey();if(expected>=0){frame=jitter.remove(expected);expected++;}}if(frame==null)frame=silence;int off=0;while(running&&off<frame.length){int n=t.write(frame,off,frame.length-off,AudioTrack.WRITE_BLOCKING);if(n<=0)break;off+=n;}}
