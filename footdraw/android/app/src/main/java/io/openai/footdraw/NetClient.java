...
public final class NetClient implements AutoCloseable {
    ...
    private void loop(){
        ...
        while(running){
            try(Socket sock=new Socket()){
                ...
                wait=120;runSocket(sock);
            }catch(Exception ignored){sleep(wait);wait=Math.min(2500,wait*2);}
    ...
    private void runSocket(Socket s)throws Exception{
        ...
        while(running&&!s.isClosed()){
            ...
            for(Store.Event e:batch){
                write(out,String.format(Locale.US,"P %d %d %s %s %.9f %.9f %.5f %d %06x",e.seq,e.epoch,e.stroke,e.kind,e.x,e.y,e.p,e.ts,e.color&0xFFFFFF));
                lastSent=e.seq;
            }
            String gc=pendingGyroCursor;
            if(gc!=null){pendingGyroCursor=null;write(out,gc);}
            while(in.ready()){
                String line=in.readLine();if(line==null)return;handle(line);
            }
            long now=System.currentTimeMillis();
            if(now-lastPing>2000){write(out,"PING");lastPing=now;}
            sleep(8);
        }
    }
    private void handle(String line){
        String[] a=line.trim().split("\\s+");if(a.length==0)return;
        try{
            switch(a[0]){
                case "STATE":
                    if(a.length>=6){
                        long e=Long.parseLong(a[1]);boolean cleared=store.syncEpoch(e);
                        double x=Double.parseDouble(a[2]),y=Double.parseDouble(a[3]);store.setView(x,y);
                        if(cleared&&listener!=null)listener.onClear(e);if(listener!=null)listener.onView(x,y);
                    }
                    break;
                case "VIEW":
                    if(a.length>=3){double x=Double.parseDouble(a[1]),y=Double.parseDouble(a[2]);store.setView(x,y);if(listener!=null)listener.onView(x,y);}
                    break;
                case "CFG":
                    if(a.length>=9&&listener!=null){
                        ...
                        float l=Float.parseFloat(a[5]),t=Float.parseFloat(a[6]),r=Float.parseFloat(a[7]),b=Float.parseFloat(a[8]);
                ...
                case "GVIEW":
                    ...
                    break;
                case "CLEAR":
                    if(a.length>=2){long e=Long.parseLong(a[1]);store.clearTo(e);if(listener!=null)listener.onClear(e);}
                    break;
                case "ACKP":
    ...
    @Override public void close(){
        running=false;store.setWake(null);
        Socket s=activeSocket;if(s!=null)try{s.close();}catch(Exception ignored){}
